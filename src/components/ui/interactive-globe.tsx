...
export function InteractiveGlobe({
  ...
  const draw = useCallback(() => {
    ...
    glowGrad.addColorStop(0.4, "rgba(60, 140, 255, 0.03)");
    glowGrad.addColorStop(1, "rgba(60, 140, 255, 0)");
    ctx.fillStyle = glowGrad;
    ctx.fillRect(0, 0, w, h);

    // Inner sphere shading (subtle fill)
    const sphereGrad = ctx.createRadialGradient(
      cx - radius * 0.3, cy - radius * 0.3, radius * 0.1,
      cx, cy, radius
    );
    sphereGrad.addColorStop(0, "rgba(40, 100, 180, 0.06)");
    sphereGrad.addColorStop(0.7, "rgba(20, 60, 120, 0.03)");
    sphereGrad.addColorStop(1, "rgba(10, 30, 60, 0)");
    ctx.beginPath();
    ...
    for (let i = 0; i < dots.length; i++) {
      ...
      const depthAlpha = Math.max(0.2, 1 - (z + radius) / (2 * radius));
      const dotSize = 1.2 + depthAlpha * 1.0;
    ...
    for (const conn of connections) {
      ...
      // Arc glow (wider, softer)
      ctx.beginPath();
      ctx.moveTo(sx1, sy1);
      ctx.quadraticCurveTo(scx, scy, sx2, sy2);
      ctx.strokeStyle = arcColor.replace(/[\d.]+\)$/, "0.15)");
      ctx.lineWidth = 4;
      ctx.stroke();
      ...
      dotGlow.addColorStop(0, markerColor.replace("1)", "0.6)"));
      dotGlow.addColorStop(1, markerColor.replace("1)", "0)"));
    ...
    for (const marker of markers) {
      ...
      // Outer glow
      const markerGlow = ctx.createRadialGradient(sx, sy, 0, sx, sy, 16);
      markerGlow.addColorStop(0, markerColor.replace("1)", "0.2)"));
      markerGlow.addColorStop(1, markerColor.replace("1)", "0)"));
      ...
      ctx.strokeStyle = markerColor.replace("1)", `${0.2 + pulse * 0.3})`);
      ctx.lineWidth = 1.5;
      ctx.stroke();
      ...
      if (marker.label) {
        ...
        ctx.fillStyle = markerColor.replace("1)", "0.85)");
        ctx.fillText(marker.label, sx + 12, sy + 4);
      }
    }
