...
export default function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className="h-screen shrink-0 border-r border-white/[0.04] bg-[#0A0E17]/95 backdrop-blur-xl flex flex-col z-50 relative transition-all duration-300 ease-in-out"
    >
      {/* Toggle Button */}
      <button
        onClick={() => setCollapsed(!collapsed)}
        className="absolute -right-3 top-6 bg-[#0A0E17] border border-white/[0.08] rounded-full p-1 text-slate-400 hover:text-white hover:bg-white/[0.05] transition-colors z-50"
      >
              ...
              <TooltipTrigger asChild>
                ...
                >
                  {item.active && (
                    <motion.div 
                      layoutId="activeTab"
                      className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-red-500 rounded-r-full shadow-[0_0_12px_rgba(239,68,68,0.8)]" 
                    />
                  )}
